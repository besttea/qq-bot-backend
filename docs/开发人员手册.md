...
4.34→4.  **初始化数据库**
    ```bash
    # 这一步至关重要，它会将 Prisma Client 生成到 libs/database/src/generated/client
    npx prisma generate --schema=libs/database/prisma/schema.prisma
    
    # 运行数据库迁移 (注意：由于 Prisma 7 的配置变更，迁移命令会自动读取 prisma.config.ts)
...
### 4.1 认证 (Auth)
100→*   `POST /auth/qq-login`: QQ 授权登录
101→    *   Payload: `{ code: "..." }`
102→    *   Response: `{ accessToken: "...", user: { ... } }`
103→    *   **Mock 测试**: 使用 `test_code_teacher` 或 `test_code_student` 作为 `code` 即可绕过真实 QQ 验证直接登录。
104→
### 4.2 作业 (Homeworks)
*   `POST /homeworks`: 发布作业
